...
const componentsFolderPath = path.join(
  __dirname,
  "..",
  "storybooks/GluestackUIPro"
);
const outputFilePath = path.join(
  __dirname,
  "..",
  "storybook-screens.config.ts"
);
...
function generateConfigText(config) {
  ...
  Object.keys(config).forEach((componentType) => {
    config[componentType].map((componentName) => {
      const componentMetaPath = `./storybooks/GluestackUIPro/${componentType}/${componentName}/${componentName}.stories`;
      const componentStoryPath = `./storybooks/GluestackUIPro/${componentType}/${componentName}/${componentName}`;
